<template>
  <div class="min-h-screen p-2">
    <div class="container mx-auto max-w-3xl pb-20 pt-12">
        <PageHeader title="Key Publications"></PageHeader>
        <div class="mt-12 space-y-10">
          <div v-for="group in groupedPublications" :key="group.year">
            <h2 class="mb-3 font-serif text-2xl font-semibold text-ink">{{ group.year }}</h2>
            <ol class="divide-y divide-line border-t border-line">
              <li v-for="pub in group.items" :key="pub.title" class="py-4">
                <p class="text-base font-medium text-ink">{{ pub.title }}</p>
                <p class="mt-1 text-sm text-ink-muted">{{ pub.description }}</p>
              </li>
            </ol>
          </div>
        </div>
...
const groupedPublications = Object.values(
  publications.reduce((groups, pub) => {
    (groups[pub.year] ||= { year: pub.year, items: [] }).items.push(pub);
    return groups;
  }, {})
